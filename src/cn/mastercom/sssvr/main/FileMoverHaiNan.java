...
public class FileMoverHaiNan extends Thread
{

	public static void main(String[] args)
	{
		FileMoverHaiNan fm = new FileMoverHaiNan();
		fm.start();
	}

	private static String HdfsRoot = "192.168.1.31";// 10.188.44.7
	...
	public void run()
	{
		...
		if (!LocalFile.checkFileExist(httpPath) && !LocalFile.checkFileExist(mmePath))
		{
			...
			System.out.println("mmePath" + mmePath);
	...
	{
		System.out.println("FileMover Hainan thread start!");
		readConfigInfo();
	}
		...
		try
		{
			...
			if (file.exists())
			{
				Document doc = reader.read(file);// 读取XML文件
				{
					...
					List<String> list5 = doc.selectNodes("//comm/HdfsRoot");
	...
	public static void MoveXdrFilesTohdfs(String httpfilePath, String mmefilePath)
	{
		...
		try
		{
			httpfiles = LocalFile.getAllFiles(new File(httpfilePath), "", 1);
		...
		if (mmefiles.size() > 0)
		{
			for (String dateStr : daysMmeMap.keySet())
			{
				...
				uploadXdrFile(daysMmeMap.get(dateStr), dateStr);
			}
		}
		if (httpfiles.size() > 0)
		{
			for (String dateStr : daysHttpMap.keySet())
			{
				...
				uploadXdrFile(daysHttpMap.get(dateStr), dateStr);
			}
		}
	}
	...
	public static void getDaysFileMap(HashMap<String, ArrayList<String>> map, List<String> files)
	{
		...
		{
			String[] nameSpliters = new File(file).getName().split("_", -1);
			if (nameSpliters.length > 4)
			{
				key = nameSpliters[4].substring(0, 10);
			}
			else if (nameSpliters.length > 1)
			{
				key = nameSpliters[1].substring(0, 10);
			}
			list = map.get(key);
			if (list == null)
			{
	...
	public static void uploadXdrFile(final List<String> files, String dateStr)
	{
		...
		for (String fileName : files)
		{
			...
			if (file.isFile())
			{
				...
				{
					Callable fm = new FileMoverCallable(fileList, dateStr);
					// 执行任务并获取Future对象
	...
	final static class FileMoverCallable implements Callable<Object>
	{
		...
		FileMoverCallable(List<String> fns, String dateStr)
		{
		...
		public Object call()
		{
			...
			try
			{
				...
				String destDir = "/seq/" + type + "/" + (dateStr.length() > 8 ? dateStr.substring(0, 8) : dateStr);
